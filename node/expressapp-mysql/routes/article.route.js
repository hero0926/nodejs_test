/**
 * Created by 207-003 on 2017-05-17.
 */
// Route 용 Express 모듈 가져오기

const express = require("express");
const fs = require('fs'); //파일 읽기 모듈
const ejs = require('ejs'); //ejs도깔거얌
const multipart = require('connect-multiparty');
const mysql = require('../db/mysql_client');

// Route를 수행하는 객체 가져오기
const router = express.Router();

//제일먼저 실행시키자!
router.use(function (request, response, next) {

    console.log(request.session.USER);

    //로그인을 안했다면?
    if(request.session.USER == undefined){

        response.redirect("/user/login");

        //얘는 끝낸다.
        return;

    }else{
        //다음 미들웨어를 실행시키자!
        next();

    }

});


//http://localhost:3000 으로 접속시 보이는 화면 설정
router.get("/", function (request, response) {


    fs.readFile('view/list.html', 'utf-8', function (err, data) {

        if(err){
            response.type("text/html");
            response.sendStatus(500);
        }

        response.type("text/html");

        mysql.query( 'select * from article ', [], function (err, results) {

            response.send(ejs.render(data, {

                title : "Node.js List Page",
                list : results

            })); //render 필수 render(본문, 변수 정의)

        });

        });



});

//글쓰기페이지
router.get("/write", function (request, response) {

    fs.readFile('view/write.html', 'utf-8', function (err, data) {

        if(err){
            response.type("text/html");
            response.sendStatus(500);
        }

        response.type("text/html");
        response.send(ejs.render(data, {

            title : "Node.js Writer Page"
        })); //render 필수 render(본문, 변수 정의)

    });

});

//글쓰기 전송 페이지(post방식!)
//얘한테만 미들웨어를 넘긴다.
router.post("/write", multipart({

        uploadDir : __dirname + "/../multipart"

    }), function (request, response) {

    //POST로 전달된 데이터들을 받아온다
    //let -> ES6
    let subject = request.body.subject;
    let writer = request.body.writer;
    let content = request.body.content;
    let files = request.files;

    console.log(files);

    //mockdata에 추가하기 위해 리터럴 생성
    const newData = {

        subject : subject,
        content : content,
        writer : writer

    };

    //파일 업로드 했을 경우
    if(files.file.originalFilename != ''){

        newData.originalFilename = files.file.originalFilename;

        //c:\users\project\hero.jpg
        path = files.file.path.split("\\");

        //hero.jpg
        path = path[path.length-1];
        console.log(path);

        newData.filePath = path;

    }

    //insert

    let query = 'insert into article (subject, content, writer, originalFileName, filePath ) values (?, ?, ?, ?, ?);';

    mysql.query(query, [

        newData.subject,
        newData.content,
        newData.writer,
        newData.originalFilename,
        newData.filePath

    ], function () {
        response.redirect("/");
    });


});

//글 읽기

router.get("/contents/:id", function (request, response) {

    //글번호 받아오기
    let id = request.params.id;

    fs.readFile("view/detail.html", "utf-8", function (err, data) {

        if (err) {
            response.type("text/html");
            response.sendStatus(500);
        }

        mysql.query("select * from article where id = ?", [id], function (err, result) {

            response.type("text/html");
            response.send(ejs.render(data, {
                
                //1개만 가져오고 싶을때는 뒤에 [0]으로 1개만 얻어온다
                item: result[0],
                id : id

            }));
        });



    });
});

//글 삭제

router.get("/delete/:id", function (request, response) {

    let id = request.params.id;

    //배열 삭제
    mysql.query("delete from article where id = ?", [id], function () {

        response.redirect("/");

    })

});




// router 객체는 express에서 사용할 수 있도록 router 모듈에 포함시킴
exports.router = router;